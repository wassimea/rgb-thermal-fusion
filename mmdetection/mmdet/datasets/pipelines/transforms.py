...
class CustomNormalize:
    """Normalize the rgb and thermal images.
    ...
    def __init__(self, 
        ...
        self.mean_grayscale = np.array(mean_grayscale, dtype=np.float32) if mean_grayscale is not None else None
        self.std_grayscale = np.array(std_grayscale, dtype=np.float32) if std_grayscale is not None else None
        self.to_rgb = to_rgb
    ...
    def __call__(self, results):
        ...
        """
        for key in results.get('img_fields', ['img']):
            mean, std, to_rgb = None, None, self.to_rgb
            if results['to_grayscale']:
                mean, std, to_rgb = self.mean_grayscale, self.std_grayscale, False
            else:
                mean, std = None, None
                if 'thermal' in results['img_prefix']:
                    mean, std = self.thermal_mean, self.thermal_std
                else:
                    mean, std = self.rgb_mean, self.rgb_std
            if self.min_max:
                cur_min, cur_max = 0, 255
                new_min, new_max = self.new_min, self.new_max
                results[key] = (results[key] - cur_min) / (cur_max - cur_min) * (new_max - new_min) + new_min
            else:
                results[key] = mmcv.imnormalize(results[key], mean, std, to_rgb)
        results['img_norm_cfg'] = dict(mean=mean, std=std, to_rgb=to_rgb)
        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        # repr_str += f'(mean={self.mean}, std={self.std}, to_rgb={self.to_rgb})'
        repr_str += f'(rgb_mean={self.rgb_mean}, rgb_std={self.rgb_std}, thermal_mean={self.thermal_mean}, thermal_std={self.thermal_std}, to_rgb={self.to_rgb})'
        return repr_str
...
class Normalize:
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            # if results['to_grayscale']:
                # results[key] = mmcv.imnormalize(results[key], self.mean_grayscale, self.std_grayscale, False)
            # else:
